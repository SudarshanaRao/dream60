...
export function PrizeShowcase({ currentPrize, onPayEntry, onPaymentFailure, onUserParticipationChange, isLoggedIn }: PrizeShowcaseProps) {
  ...
  const fetchServerTime = async () => {
    try {
      ...
      if (data.success && data.data) {
        ...
        // ✅ Store server time baseline with local timestamp for accurate countdown
        ...
        // Check if we're within the first 15 minutes (0-14 minutes to match backend validation)
        const isWithinJoinWindow = data.data.minute < 15;
        setIsJoinWindowOpen(isWithinJoinWindow);
      }
    } catch (error) {
      console.error('Error fetching server time:', error);
      // Fallback: allow joining if we can't fetch server time
  ...
  const getCurrentServerTime = (): number => {
    if (!serverTimeBaseline) {
      // Fallback to local IST time if server time not available
  ...
  const calculateAuctionEndTime = (rounds: Round[]) => {
    ...
    if (!rounds || rounds.length === 0) {
      console.log('⚠️ [CALCULATE END TIME] No rounds data, using fallback (+60 min)');
    ...
    // Fallback: use server time + 60 minutes
    console.log('⚠️ [CALCULATE END TIME] No completedAt found, using fallback (+60 min)');
    return new Date(getCurrentServerTime() + 60 * 60 * 1000);
  };
  ...
  useEffect(() => {
    fetchLiveAuction(true); // Initial load with loading spinner

    // Poll live auction data every 5 seconds to update participants count in real-time
    const auctionInterval = setInterval(() => fetchLiveAuction(false), 5000);

    // ✅ REMOVED: Server time interval - now using prop from parent
    return () => {
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      ...
      // ✅ Detailed console logs for time calculation
      ...
      const endTimeIST = auctionEndTime.toLocaleString('en-IN', { 
        timeZone: 'Asia/Kolkata',
        year: 'numeric',
        month: '2-digit',
        day: '2-digit',
      ...
      console.log('⏱️ [COUNTDOWN TIMER]', {
        'Round 4 Ends At (IST)': endTimeIST,
        'Current Server Time (IST)': nowIST,
        'Round 4 Ends At (Timestamp)': auctionEndTime.getTime(),
        'Current Server Time (Timestamp)': now,
        'Time Difference (ms)': distance,
        'Time Difference (minutes)': Math.floor(distance / (1000 * 60)),
        'Countdown Display': distance > 0 ? `${Math.floor((distance % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60))}:${Math.floor((distance % (1000 * 60 * 60)) / (1000 * 60))}:${Math.floor((distance % (1000 * 60)) / 1000)}` : '00:00:00'
      });
  ...
  }, [auctionEndTime, serverTimeBaseline]);
        ...
        <div className="relative backdrop-blur-md bg-white/30 rounded-[24px] p-2 sm:p-3 md:p-4 border border-white/60 shadow-2xl">
          <div className="grid md:grid-cols-2 gap-3 sm:gap-4 md:gap-5 items-stretch">
            ...
            <div className="relative order-first md:order-last space-y-2 sm:space-y-3 md:space-y-4 h-full flex flex-col">
              ...
              {/* Image Card – aligned like Pay Entry Fee to Join card */}
              <div className="relative group/image flex-1 flex flex-col">
                ...
                <div className="relative overflow-hidden rounded-2xl backdrop-blur-2xl bg-white/75 
    border border-purple-200/50 p-2 sm:p-3 md:p-4 shadow-2xl flex-1 flex">
...
export function PrizeShowcase({ currentPrize, onPayEntry, onPaymentFailure, onUserParticipationChange, isLoggedIn }: PrizeShowcaseProps) {
  ...
  useEffect(() => {
    ...
    // Refresh server time every 30 seconds to keep join window status updated
...
interface PrizeShowcaseProps {
  ...
  onUserParticipationChange?: (isParticipating: boolean) => void;
  isLoggedIn?: boolean;
  serverTime?: ServerTime | null; // ✅ NEW: Receive server time from parent
}

export function PrizeShowcase({ currentPrize, onPayEntry, onPaymentFailure, onUserParticipationChange, isLoggedIn, serverTime }: PrizeShowcaseProps) {
  const [liveAuctions, setLiveAuctions] = useState<AuctionConfig[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [boxAFee, setBoxAFee] = useState<number>(0);
  const [boxBFee, setBoxBFee] = useState<number>(0);
  ...
  // ✅ REMOVED: fetchServerTime function - now using prop from parent
  // ✅ REMOVED: serverTimeBaseline state - using prop directly

  // ✅ Update join window status when serverTime prop changes
  useEffect(() => {
    if (serverTime) {
  ...
  // ✅ Calculate current server time based on prop
  const getCurrentServerTime = (): number => {
    if (!serverTime) {
      return Date.now();
    }
